...
case $OS_VERSION in
  ...
  *) warn "Sorry! You'll have to upgrade your OS to $MINIMUM_MAC_OS or above."; exit 1;;
esac
...
if [ ! -z $(pkgutil --pkgs=com.apple.pkg.$cmdline_version) ]; then
  ...
  warn "  When you're done rerun this script (the installfest)..."
  ...
  warn "  and then rerun this script (the installfest)..."
  exit 1
else
  warn "Command Line Tools are not installed!"
  warn "  Running 'xcode-select --install' Please click continue!"
  warn "  After installing please rerun this script (the installfest)..."
